...
for fichero in $fuentes; do
 ...
 test $fichero == Localidad.java || test $fichero == Localidad2.java || \
 test $fichero == TNear.java || test $fichero == GrafLoc.java || \
 test $fichero == InfoTur.java || test $fichero == Prim.java ; then 
...
if test $numfuentes -eq 8 -a $javas -eq 8; then
  ...
  continuar=false
  echo "Error, ficheros fuente requeridos: Localidad.java, Coordenadas.java, Coleccion.java, InfoTur.java, Localidad2.java, GrafLoc.java, TNear.java y Prim.java; 0"
fi
